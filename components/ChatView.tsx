...
const ChatView: React.FC = () => {
    ...
    const handleSend = (e: React.FormEvent) => {
        e.preventDefault();
        if (!inputText.trim() || !activeConversation) return; // Note: !inputText.trim() check prevents sending empty messages, but allow links
    ...
    const handleCreateMeeting = () => {
        ...
        const meetingLink = `https://meet.google.com/${segment(3)}-${segment(4)}-${segment(3)}`;
        setInputText(meetingLink);
    };
    ...
    return (
        <div className="flex h-[calc(100vh-64px)] bg-slate-50 dark:bg-slate-950 overflow-hidden animate-in fade-in duration-500">
            {/* Sidebar */}
                    ...
                    {conversations.map(conv => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 p-3 rounded-2xl transition-all ${active ? 'bg-emerald-50 dark:bg-emerald-900/20 ring-1 ring-emerald-100 dark:ring-emerald-900/50 shadow-sm shadow-emerald-100 dark:shadow-none' : 'hover:bg-slate-50 dark:hover:bg-slate-800'
                                    }`}
                            >
                                <div className="relative flex-shrink-0">
                                    <div className={`w-12 h-12 rounded-full border flex items-center justify-center font-bold overflow-hidden ${active ? 'bg-white dark:bg-slate-800 border-emerald-200 dark:border-emerald-800 shadow-sm' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500'
                                        }`}>
                                ...
                                <div className="flex-1 text-left min-w-0">
                                    <div className="flex justify-between items-baseline mb-0.5">
                                        <span className={`font-bold truncate text-sm ${active ? 'text-emerald-900 dark:text-emerald-100' : 'text-slate-900 dark:text-white'}`}>
                                    ...
                                    <p className={`text-xs truncate font-medium ${active ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
            ...
            <div className="flex-1 flex flex-col bg-white dark:bg-slate-900">
                {activeConversation ? (
                    <>
                        {/* Header */}
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between shadow-sm bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-10">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-slate-900 dark:text-white">{activeConversation.type === 'GROUP' ? activeConversation.name : getPartner(activeConversation)?.name}</h3>
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group animate-in fade-in slide-in-from-bottom-2`}>
                                        <div className={`max-w-[70%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                            ...
                                            <div className={`px-4 py-3 rounded-2xl text-sm shadow-sm transition-all hover:shadow-md ${isMe ? 'bg-emerald-600 text-white rounded-tr-none' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-tl-none'
                                                }`}>
                                                {msg.text}
                                            </div>
                        ...
                        <form onSubmit={handleSend} className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
                            <div className="relative flex items-center bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-1 focus-within:ring-2 ring-emerald-500/20 transition-all focus-within:bg-white dark:focus-within:bg-slate-800 focus-within:border-emerald-200 dark:focus-within:border-emerald-800">
                                <button type="button" className="p-2 text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-xl transition-colors">
                                    <ICONS.Paperclip className="w-5 h-5" />
                                </button>
                                <button type="button" onClick={handleCreateMeeting} className="p-2 text-slate-400 hover:text-red-500 dark:hover:text-red-400 rounded-xl transition-colors" title="Create Google Meet">
                                ...
                                <input
                                    ...
                                    className="flex-1 bg-transparent border-none focus:ring-0 text-sm p-3 placeholder:text-slate-400 outline-none text-slate-900 dark:text-white"
                                />
                                <button
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-slate-50/30 dark:bg-slate-900/30">
                        <div className="w-24 h-24 bg-emerald-50 dark:bg-emerald-900/20 rounded-[2.5rem] flex items-center justify-center mb-8 ring-8 ring-emerald-500/5 pulse">
                            <ICONS.MessageSquare className="w-12 h-12 text-emerald-600" />
                        </div>
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Your Direct Workspace</h3>
                        <p className="text-slate-500 dark:text-slate-400 text-sm max-w-xs mt-3 font-medium leading-relaxed">Select a teammate or client to start chatting or create a group for team coordination.</p>
                        <button
                            onClick={() => setShowNewChat(true)}
            ...
            {/* New Chat Modal */}
            {
                showNewChat && (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                        <div className="bg-white dark:bg-slate-900 rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20 dark:border-slate-800">
                            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                                <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight text-sm">New Conversation</h2>
                                <button onClick={() => { setShowNewChat(false); setSelectedUsers([]); }} className="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                            ...
                            <div className="p-6">
                                {selectedUsers.length > 1 && (
                                    <div className="mb-4 animate-in slide-in-from-top-4 duration-300">
                                        ...
                                        <input
                                            ...
                                            className="w-full px-4 py-3.5 bg-indigo-50/30 dark:bg-indigo-900/20 border-2 border-indigo-100 dark:border-indigo-800 rounded-2xl text-sm focus:ring-4 ring-indigo-500/10 focus:border-indigo-300 outline-none transition-all placeholder:text-indigo-300 font-bold text-slate-900 dark:text-white"
                                            value={groupName}
                                ...
                                <div className="relative mb-6">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl text-sm focus:ring-4 ring-emerald-500/5 focus:border-emerald-200 dark:focus:border-emerald-800 outline-none transition-all font-medium text-slate-900 dark:text-white"
                                        value={searchTerm}
                                ...
                                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                                    {filteredUsers.map(u => (
                                        <button
                                            ...
                                            className={`w-full flex items-center gap-4 p-3.5 rounded-2xl transition-all group ${selectedUsers.includes(u.id) ? 'bg-emerald-50 dark:bg-emerald-900/20 ring-1 ring-emerald-100 dark:ring-emerald-900/50' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                                        >
                                            <div className="relative">
                                                <div className="w-11 h-11 rounded-full bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 flex items-center justify-center overflow-hidden shadow-sm group-hover:scale-105 transition-transform">
                                            ...
                                            <div className="text-left flex-1 min-w-0">
                                                <h4 className="font-bold text-slate-900 dark:text-white text-sm truncate group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">{u.name}</h4>
                                                <p className="text-[9px] text-slate-400 dark:text-slate-500 font-black uppercase tracking-wider">{u.role}</p>
                                            </div>
                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${selectedUsers.includes(u.id) ? 'bg-emerald-500 border-emerald-500 shadow-lg shadow-emerald-200 dark:shadow-none' : 'border-slate-200 dark:border-slate-700'}`}>
